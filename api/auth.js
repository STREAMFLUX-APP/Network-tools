...
export default async function handler(req, res) {
  ...
  if (action === "login") {
    ...
    // Simple password check (passwords stored as plain text for simplicity — upgrade to bcrypt later)
    if (user.password !== password) return res.status(401).json({ error: "Invalid email or password" });

    if (!user.active) return res.status(403).json({ error: "Your account has been deactivated. Please contact support." });
    ...
    if (!trialActive && !subscriptionActive) {
      return res.status(403).json({
        error: "Your 14-day free trial has ended. Please subscribe to continue.",
        trialExpired: true
      });
    }
